...
import lavamystic
from lavamystic.utils import ExtrasNamespace
from lavamystic.exceptions import (
    ChannelTimeoutException,
    InvalidChannelStateException,
    LavalinkException,
    QueueEmpty,
)
from lavamystic.filters import Filters
from lavamystic.node import Pool
from lavamystic.payloads import (
    PlayerUpdateEventPayload,
    TrackEndEventPayload,
)
from lavamystic.queue import Queue
from lavamystic.tracks import Playable, Playlist
...
class Player(disnake.VoiceProtocol):
    ...
    def dispatch_message_update(self) -> None:
        ...
        self.client.dispatch("mystic_message_update", self)
    ...
    async def auto_play_event(self, payload: TrackEndEventPayload) -> None:
        if self._autoplay is lavamystic.AutoPlayMode.disabled:
        ...
        if self.node.status is not lavamystic.NodeStatus.CONNECTED:
        ...
        if self.queue.mode is lavamystic.QueueMode.loop:
            await self._do_partial(history=False)

        elif self.queue.mode is lavamystic.QueueMode.loop_all:
            await self._do_partial()

        elif self._autoplay is lavamystic.AutoPlayMode.partial or self.queue:
            await self._do_partial()
    ...
    def inactive_timeout(self) -> int | None:
        ...
        :func:`on_mystic_inactive_player` event.
        ...
        :func:`on_mystic_inactive_player` event or ``None`` to remove the timeout.
        ...
        - See: :class:`~Node`
        - See: :func:`on_mystic_inactive_player`
        """
    ...
    @property
    def autoplay(self) -> lavamystic.AutoPlayMode:
    ...
    def autoplay(self, value: Any) -> None:
        if not isinstance(value, lavamystic.AutoPlayMode):
    ...
    async def destroy(self) -> None:
        ...
        self.node.client.dispatch("mystic_player_destroyed", player)
